...
func printUpdateTextCommon(version semver.Version) {
	...
	url := "https://github.com/nholuongut/minikube/releases/tag/v" + version.String()
...
func DownloadURL(ver, os, arch string) string {
	if ver == "" || strings.HasSuffix(ver, "-unset") || os == "" || arch == "" {
		return "https://github.com/nholuongut/minikube/releases"
	}
	sfx := ""
	if os == "windows" {
		sfx = ".exe"
	}
	return fmt.Sprintf("https://github.com/nholuongut/minikube/releases/download/%s/minikube-%s-%s%s",
		ver, os, arch, sfx)
}
