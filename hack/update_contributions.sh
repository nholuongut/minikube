...
while read -r tag_index tag_name tag_start tag_end; do
  ...
  "${DIR}/pullsheet" leaderboard --token-path "$TMP_TOKEN" --repos nholuongut/minikube --since "$tag_start" --until "$tag_end" --hide-command --logtostderr=false --stderrthreshold=2 \
    >> "$destination/$tag_name.html"
