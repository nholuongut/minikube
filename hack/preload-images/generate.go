...
func generateTarball(kubernetesVersion, containerRuntime, tarballFilename string) error {
	...
	if containerRuntime != "docker" { // kic overlay image is only needed by containerd and cri-o https://github.com/nholuongut/minikube/issues/7428
...
func deleteMinikube() error {
	cmd := exec.Command(minikubePath, "delete", "-p", profile) // to avoid https://github.com/nholuongut/minikube/issues/7814
	cmd.Stdout = os.Stdout
	return cmd.Run()
}
