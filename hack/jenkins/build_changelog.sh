...
OUTPUT_LOCATION=${1}
gh search prs --merged --sort updated --limit 100 --repo nholuongut/minikube --json number,title,closedAt --template '{{range .}}{{tablerow (printf "#%v" .number | autocolor "green") .title (timeago .closedAt)}}{{end}}' > "${OUTPUT_LOCATION}"