...
function retry_github_status() {
  ...
  while [[ "${attempt}" -lt 8 ]]; do
    ...
    code=$(curl -o "${out}" -s --write-out "%{http_code}" -L -u minikube-bot:${token} \
      "https://api.github.com/repos/nholuongut/minikube/statuses/${commit}" \
...
# skipping gvisor mac because ofg https://github.com/nholuongut/minikube/issues/5137
if [ "$(uname)" != "Darwin" ]; then
...
then
	echo "Saving to DB"
	gopogh -in "${JSON_OUT}" -out_html "${HTML_OUT}" -out_summary "${SUMMARY_OUT}" -name "${JOB_NAME}" -pr "${MINIKUBE_LOCATION}" -repo github.com/nholuongut/minikube/  -details "${COMMIT}:$(date +%Y-%m-%d):${ROOT_JOB_ID}" -db_backend "${GOPOGH_DB_BACKEND}" -db_host "${GOPOGH_DB_HOST}" -db_path "${GOPOGH_DB_PATH}" -use_cloudsql -use_iam_auth
	echo "Exit code: $?"
else
	echo "Not saving to DB"
	gopogh -in "${JSON_OUT}" -out_html "${HTML_OUT}" -out_summary "${SUMMARY_OUT}" -name "${JOB_NAME}" -pr "${MINIKUBE_LOCATION}" -repo github.com/nholuongut/minikube/  -details "${COMMIT}:$(date +%Y-%m-%d):${ROOT_JOB_ID}"
fi
