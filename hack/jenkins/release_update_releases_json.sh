...
if ! [[ "${VERSION_BUILD}" =~ ^[0-9]+$ ]]; then
  ...
  # Send PR from minikube-bot/minikube to nholuongut/minikube
  curl -X POST -u minikube-bot:${BOT_PASSWORD} -k   -d "{\"title\": \"Post-release: update releases-beta.json & releases-beta-v2.json to include ${TAGNAME}\",\"head\": \"minikube-bot:jenkins-releases.json-${TAGNAME}\",\"base\": \"master\"}" https://api.github.com/repos/nholuongut/minikube/pulls
  ...
  # Send PR from minikube-bot/minikube to nholuongut/minikube
  curl -X POST -u minikube-bot:${BOT_PASSWORD} -k   -d "{\"title\": \"Post-release: update releases.json & releases-v2.json to include ${TAGNAME}\",\"head\": \"minikube-bot:jenkins-releases.json-${TAGNAME}\",\"base\": \"master\"}" https://api.github.com/repos/nholuongut/minikube/pulls
